...
export default function Orbits() {
  ...
  return <div style={s.page}>
    <header style={s.header}><div><p style={s.eyebrow}>Together</p><h1 style={s.title}>Shared Orbits</h1><p style={s.subtitle}>Small, private groups for building momentum together.</p></div><button style={s.button} onClick={() => navigate("/orbits/new")}>Create Orbit</button></header>
    ...
    {loading ? <p style={s.muted}>Loading Shared Orbits...</p> : orbits.length === 0 ? <div style={{...s.card, ...s.empty}}><Users size={42}/><h3 style={{color:"var(--text)"}}>No Shared Orbits yet</h3><p>Create one or accept an invite to start building momentum together.</p></div> : <div style={s.cardStack}>{orbits.map(orbit => <button key={orbit.id} onClick={() => navigate(`/orbits/${orbit.id}`)} style={{...s.card, ...s.row, width:"100%", textAlign:"left", cursor:"pointer"}}><div><h3 style={s.name}>{orbit.name}</h3><p style={s.muted}>{orbit.member_count} member{orbit.member_count === 1 ? "" : "s"} · {orbit.viewer_role}</p></div><ChevronRight color="var(--muted)"/></button>)}</div>}
  </div>;
}
