...
export default function OrbitMembers() {
  ...
  return <div style={s.page}><header style={s.header}><div><p style={s.eyebrow}>Shared Orbit</p><h1 style={s.title}>Members</h1><p style={s.subtitle}>{orbit.member_count} people in {orbit.name}</p></div><button style={s.secondaryButton} onClick={() => navigate(`/orbits/${orbitId}`)}>Back</button></header>
    {canManage && <div style={s.card}><h2 style={{...s.name, fontSize:22}}>Invite someone</h2><div style={{display:"flex", alignItems:"center", gap:10, marginTop:16}}><Search size={19} color="var(--muted)"/><input style={{...s.input, border:0, paddingLeft:0}} value={query} onChange={e => setQuery(e.target.value)} placeholder="Search by username or name"/></div>{results.length > 0 && <div style={{...s.cardStack, marginTop:12}}>{results.map(user => <button key={user.id} onClick={() => invite(user)} disabled={busy === user.id} style={{...s.row, border:0, borderTop:"1px solid var(--border)", background:"transparent", padding:"13px 0 0", cursor:"pointer", textAlign:"left"}}><div style={{display:"flex", alignItems:"center", gap:12}}><UserAvatar user={user} size={34}/><div><strong style={{color:"var(--text)"}}>{user.display_name || user.name || user.username}</strong><p style={{...s.muted, marginTop:2}}>@{user.username}</p></div></div><Plus color="var(--primary)"/></button>)}</div>}</div>}
    <h2 style={s.sectionTitle}>Current members</h2><div style={s.cardStack}>{(orbit.members || []).map(member => <div key={member.user_id} style={{...s.card, ...s.row}}><div style={{display:"flex", alignItems:"center", gap:12}}><UserAvatar user={member.user} size={38}/><div><h3 style={{...s.name, fontSize:17}}>{member.user?.display_name || member.user?.name || "Member"}</h3><p style={{...s.muted, marginTop:3, textTransform:"capitalize"}}>{member.role}</p></div></div>{canManage && member.role !== "owner" && <button style={s.dangerButton} disabled={busy === member.user_id} onClick={() => remove(member)}>Remove</button>}</div>)}</div>
